<div wire:poll.10000ms class="page-transition">
    ...
    <div class="flex justify-between items-start mb-10">
        ...
        <div class="flex items-center gap-2 bg-[#FCEEE5] border border-[#FCEEE5] px-4 py-2 rounded-full text-[#9E460B] text-xs font-bold shadow-sm cursor-pointer hover:bg-[#FDF2EA] transition">
            ...
            <span>Hari ini, {{ now()->format('d M') }}</span>
        </div>
    </div>

    <!-- Overview Header -->
    <div class="mb-8">
        <h2 class="text-2xl font-extrabold text-[#331C0E] font-display">Ringkasan</h2>
        <p class="text-sm text-[#8A7160] mt-1">Inilah yang terjadi di kampus hari ini.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
        ...
        <div class="bg-[#8E4E14] rounded-[2rem] p-8 text-white shadow-sm flex flex-col justify-between h-56 relative overflow-hidden card-hover animate-fade-in-up">
            ...
            <div class="mt-4">
                <p class="text-[10px] font-extrabold uppercase tracking-widest text-[#FFF1E5]/80">Pesanan Hari Ini</p>
        ...
        <div class="bg-white border border-[#FCEEE5] rounded-[2rem] p-8 shadow-sm flex flex-col justify-between h-56 card-hover animate-fade-in-up delay-150">
            ...
            <div class="mt-4">
                <p class="text-[10px] font-extrabold uppercase tracking-widest text-[#8A7160]">Total Mahasiswa</p>
        ...
        <div class="bg-white border border-[#FCEEE5] rounded-[2rem] p-8 shadow-sm flex flex-col justify-between h-56 card-hover animate-fade-in-up delay-300">
            ...
            <div class="mt-4">
                <p class="text-[10px] font-extrabold uppercase tracking-widest text-[#8A7160]">Selesai (Hari Ini)</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
        ...
        <div class="bg-white border border-[#FCEEE5] rounded-[2rem] p-6 shadow-sm flex items-center gap-4 lg:col-span-1">
            ...
            <div class="min-w-0">
                <p class="text-[10px] font-extrabold uppercase tracking-widest text-[#8A7160]">Pendapatan Hari Ini</p>
        ...
        <div class="bg-white border border-[#FCEEE5] rounded-[2rem] p-6 shadow-sm flex items-center gap-4 lg:col-span-1">
            ...
            <div class="min-w-0">
                <p class="text-[10px] font-extrabold uppercase tracking-widest text-[#8A7160]">Total Penjual</p>
        ...
        <div class="bg-white border border-[#FCEEE5] rounded-[2rem] p-6 shadow-sm flex items-center gap-4 lg:col-span-1">
            ...
            <div class="min-w-0">
                <p class="text-[10px] font-extrabold uppercase tracking-widest text-[#8A7160]">Aktif Sekarang</p>
        ...
        <div class="bg-white border border-red-100 rounded-[2rem] p-6 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 lg:col-span-2">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-[10px] font-extrabold uppercase tracking-widest text-[#8A7160]">Ditolak/Dibatalkan Hari Ini</p>
                    <p class="text-2xl font-extrabold text-[#331C0E] font-display mt-0.5">{{ $rejectedToday }}</p>
                </div>
            </div>
            <div class="sm:text-right">
                @if($rejectedToday > 0)
                    <p class="text-xs text-red-600 font-semibold max-w-[200px]">Perlu perhatian. Terutama karena item tidak tersedia.</p>
                @else
                    <p class="text-xs text-green-600 font-semibold max-w-[200px]">Aman hari ini. Tidak ada pesanan ditolak atau dibatalkan.</p>
                @endif
            </div>
        </div>
    </div>
    ...
    @if($pendingVerification > 0)
        <div class="bg-[#FFF1E5] border border-[#FCEEE5] rounded-[2rem] p-6 mb-8 flex items-center justify-between shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm font-extrabold text-[#331C0E]">{{ $pendingVerification }} Penjual Menunggu Verifikasi</p>
                    <p class="text-xs text-[#8A7160]">Tinjau dan setujui pendaftaran penjual agar dapat mulai berjualan.</p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-[2rem] border border-[#FCEEE5] p-8 shadow-sm animate-fade-in-up delay-200">
        <div class="flex justify-between items-center mb-8">
            <div>
                ...
                <p class="text-xs text-[#8A7160] mt-1">8 event sistem terakhir dari log audit.</p>
            </div>
        ...
        @if(count($activities) === 0)
            <div class="text-center py-12 text-[#8A7160]">
                ...
                <p class="text-sm font-bold mt-2">Belum ada aktivitas yang dicatat.</p>
            </div>
        @else
                        ...
                        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 flex-wrap">
                                    ...
                                    <span class="text-[10px] text-[#8A7160] font-semibold">oleh {{ $activity['user'] }}</span>
                                </div>
